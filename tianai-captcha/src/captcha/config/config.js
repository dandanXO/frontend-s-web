...
class CaptchaConfig {
    ...
    requestCaptchaData() {
        ...
        // 设置默认值
        requestParam.headers["Content-Type"] = "application/x-www-form-urlencoded";
        requestParam.method="POST";
        requestParam.url = this.requestCaptchaDataUrl;
        const urlParams = new URLSearchParams();
        for (const key in this.loginData) {
            if (this.loginData.hasOwnProperty(key)) {
                urlParams.append(key, this.loginData[key]);
            }
        }
        requestParam.data = urlParams.toString();
        // 请求前装载参数
    ...
    validCaptcha(currentCaptchaId, data, c, tac) {
        let sendParam = this.loginData;
        sendParam.captchaTrackId = currentCaptchaId;
        sendParam.captchaTrackData = JSON.stringify(data);
        let requestParam = {};
        requestParam.headers = this.requestHeaders || "";
        const urlParams = new URLSearchParams();
        for (const key in sendParam) {
            if (sendParam.hasOwnProperty(key)) {
                urlParams.append(key, sendParam[key]);
            }
        }
        requestParam.data = urlParams.toString();
        requestParam.headers["Content-Type"] = "application/x-www-form-urlencoded";
        ...
        return request.then(res => {
            ...
            if (res.code === 0) {
                ...
                const successMsg = tac.style.i18n.tips_success.replace("{s}", useTimes);
                c.showTips(successMsg, 1, () => this.validSuccess(res, c, tac));
            } else if (res.code === 800) {
                let tipMsg = tac.style.i18n.tips_error;
                c.showTips(tipMsg, 0, () => this.validFail(res, c, tac));
            } else {
                let tipMsg = res.message;
                const translateMsg = this.translate(res.code)
                c.showTips(translateMsg ?? tipMsg, 0, () => this.validFail(res, c, tac));
            }
        }).catch(e => {
            if (e.code && e.code != 0) {
                let tipMsg = tac.style.i18n.tips_error;
