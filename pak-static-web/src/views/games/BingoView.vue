...
export default defineComponent({
  ...
  setup() {
    ...
    const switchPlat = (plat) => {
      router.push({path: 'bingo', query: {plat: plat.code}})
    ...
    const loadBanner = () => {
      loadPromoBanner("BINGO").then((res) => {
        if (res.code === 0) {
    ...
    const loadGameList = () => {
      getPlatformGames(selectedPlatId.value, "BINGO").then((data) => {
        data.forEach(element => {
          element.default = require("../../assets/images/games/aviator/default.png");
          element.icon = `${process.env.VUE_APP_IMAGE_CDN}/games/bingo/${selectedPlat.value.code}/${element.icon}.png`;
        });
