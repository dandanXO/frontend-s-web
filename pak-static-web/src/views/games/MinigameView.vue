...
export default defineComponent({
  ...
  setup() {
    ...
    const switchPlat = (plat) => {
      router.push({path: 'minigame', query: {plat: plat.code}})
    ...
    const getPlatList = () => {
      getPlatformList().then((data) => {
        platforms.value = data.filter(element => element.gameType.includes("CASUAL"));
        if (!route.query.plat) {
    ...
    const loadBanner = () => {
      loadPromoBanner("CASUAL").then((res) => {
        if (res.code === 0) {
    ...
    const loadGameList = () => {
      getPlatformGames(selectedPlatId.value, "CASUAL").then((data) => {
        data.forEach(element => {
          element.default = require("../../assets/images/games/aviator/default.png");
          element.icon = `${process.env.VUE_APP_IMAGE_CDN}/game/1/${selectedPlat.value.code.toLowerCase()}/${element.icon}.png`;
        });
