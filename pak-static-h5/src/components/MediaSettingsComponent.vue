<template>
  <div>
    <div v-for="(media, index) in filteredMediaSettings" :key="index" class="video-container">
      <iframe
...
const getVideoUrl = (media) => {
  if (media) {
    ...
    if (videoId) {
      url = `https://www.youtube.com/embed/${videoId}?autoplay=${autoplay}&mute=${mute}`;
    }
    return url;
  }
  return "";
