        ...
        <div class="player-guide" :class="`${localType}-mode`">
            <div class="steps-portion" :class="`steptopmain-${currentAdditionalStep} ${localType}-mode`">
                <div class="top">
                    ...
                    <div class="title">{{ $t('playerGuide.recharging') }}</div>
                </div>
                <div class="main-box">
                    <template v-for="(step, index) in currentSteps" :key="index">
                        <div class="other-steps" v-if="currentAdditionalStep === step.step">
                ...
                <div @click="updateCurrentAdditionalStep" class="bottom-button">
                    {{ $t('playerGuide.next') }}
                    </div>
                </div>
                </div>
            </div>
            <div class="green-highlight-box" :class="`step-${currentAdditionalStep} ${localType}-mode`" @click="updateCurrentAdditionalStep">
...
const props = defineProps({ currentAdditionalStep: Number, currentType: String });
  ...
  const localType = ref(props.currentType);
    // Watch for prop changes to keep localStep in sync
watch(() => props.currentAdditionalStep, (newVal) => {
...
const additionalReferSteps = ref([
  ...
  { step: 3, instruction: "Copy your referral link" },
  ...
  const updateCurrentAdditionalStep = () => {
    ...
    if (localType.value === 'refer') {
        if (localStep.value === 2) {
            router.push("/earn-money?isNewPlayer=1");
            localStep.value += 1;
            emit("updateStep", localStep.value); 
            return;
        }
    ...
    if (localType.value === 'withdraw') {
        if (localStep.value === 1) {
            router.push("/withdraw?isNewPlayer=1");
            localStep.value += 1;
            emit("updateStep", localStep.value); 
            return;
        }
