...
export function claimItems(status, level) {
  ...
  if (status === 'retain') {return eventapi.post("/vip-bonus/claim-first-retain?_method=PUT", { vipLevel: level })}
  if (status === 'monthly') {return eventapi.put("/vip-bonus/claim-monthly-bonus");}
  if (status === 'yearlyRetain') {return eventapi.post("/vip-bonus/claim-yearly-retain?_method=PUT", { vipLevel: level })}
  if (status === 'coupon') {return eventapi.put("/vip-bonus/claim-coupon");}
  if (status === 'all') {return eventapi.post("/vip-bonus/claim-all?_method=PUT", { vipLevel: level })}
}
