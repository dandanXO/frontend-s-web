<template>
  <q-dialog v-model="appliedRecordsModalVisible" title="申请记录">
    <q-table title="申请记录" :rows="appliedRecords" :columns="columns" row-key="name" />
  </q-dialog>
</template>
...
const columns = [
  { name: "amount", align: "center", label: "奖金金额", field: (row) => row.amount, format: (val) => `${val}` },
  {
    name: "recordTime",
    label: "领取时间",
    field: (row) => row.recordTime,
    format: (recordTime = "") => {
