<template>
    <div>
        ...
        <button class="common-btn apply-btn" @click="applySportInsurance()"
            :disabled="isNaN(sportInsuranceFormData.gameMatchId)">
            点击申请</button>

        <el-dialog v-model="isSportInsuranceModalVisible" title="电竞保险" width="100%" align-center style="max-width: 500px"
            @close="toggleSportInsuranceModal(false)">
            <div class="sport-insurance-modal-container">
                <el-form label-width="100px" id="sport-insurance-form" :rules="sportInsuranceFormValidationRules"
                    ref="sportInsuranceFormRef" :model="sportInsuranceFormData">
                    <el-form-item prop="platform" name="platform" label="投注平台: ">
                        <el-select v-model="sportInsuranceFormData.platform" placeholder="投注平台"
                            @focus="loadSportPlatformOptions()" clearable>
                            <el-option v-for="platform in sportPlatformOptions" :key="platform" :value="platform"
                                :label="platform">
                                {{ platform }}
                            </el-option>
                        </el-select>
                    ...
                    <el-button :loading="loadingBtn" size="large" @click="submitForm(sportInsuranceFormRef)"
                        class="common-btn">
                        确定
                    </el-button>
                </el-form>
...
import { getUpcomingSportMatches, getSportInsurancePlatformOptions, submitSportInsuranceForm } from "@/api/promotion/sportSafety";
