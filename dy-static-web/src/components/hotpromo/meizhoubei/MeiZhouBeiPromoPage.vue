<template>
  <div>
    <template v-if="matchDetails.length > 0">
      <swiper
...
import {
  getUpcomingCopaMatches,
  getCopaPlatformOptions,
  submitCopaForm,
...
const loadSportPlatformOptions = () => {
  ...
  getCopaPlatformOptions().then((res) => {
...
const applySportInsurance = async () => {
...
const iconImageBasePath = useLocalStorage("IMAGE_CDN", process.env.VUE_APP_IMAGE_CDN).value + '/promo';
