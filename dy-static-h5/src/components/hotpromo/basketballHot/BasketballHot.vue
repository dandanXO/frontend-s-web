        ...
        <q-tab-panel name="asia-war-guide">
          <div class="tab1">
            ...
            <div>
              ...
              <div>
                <p>1. 本活动计算IM体育的NBA&CBA当日全部赛事有效投注，每日达到对应有效投注即可领取对应流水彩金。</p>
                <p>
                  2.
                  满足活动条件的会员需在当日的23:59:59前登录该页面并点击【立即申请】按钮进行活动申请，逾事不予补偿，申请成功后系统将于次日24小时内自动派发至会员钱包，彩金仅需5倍流水即可取款。
                </p>
                <p>3. 流水仅计算当天下注且结算的注单，若提前兑现、注单未结算、取消或走盘皆不计算为有效投注。</p>
                <p>
                  4.
...
const handleClaim = (applyPromoCode, applyPromoTitle, applyGameType) => {
  eventapi
    .post(
      "/promo-application/apply",
      qs.stringify({ promoCode: applyPromoCode, promoTitle: applyPromoTitle, gameType: applyGameType })
    )
    .then((res) => {
      if (res.code === 0) {
        $q.notify({
