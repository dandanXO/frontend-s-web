        ...
        <div class="transferamounts q-my-md">
          ...
          >
            ...
            <template v-slot:append>
             <span style="font-size:26px;" class="text-bright"><q-btn label="最大金额" @click="updateTransferAmt" color="dyblue" /></span>
            </template>
          </q-input>
...
import { useQuasar } from "quasar";
              ...
              if (platform.id === transferFrom.value) {
                if (platform.code) {
                    const obj = {
                      ...
                      api.post("/session/balance/transfer/withdraw", qs.stringify(obj)).then((response) => {
                        if (response.code === 0) {
                          setTimeout(() => {
                            $q.notify({
                              color: "positive",
  ...
  const getPlatBalances = (plat) => {
    ...
    api.get('/session/balance', {params: {
      platform: plat}}).then((res) => {
            if (platform) {
