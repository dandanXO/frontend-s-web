<template>
    <q-dialog v-model="_modelValue" @hide="hideCashOutPopup" @show="handleDialogShow">
        ...
        <div class="cash-out" v-else>
            <GradientTextAmount v-if="isShowTextAmount" :amountText="`Custos de Saque ${store.currency.value} ${extractionDifference}`" />
            <div v-else class="text-amount-placeholder"></div>
            <span class="next-spin-remaining-time">Próxima Rodada: {{ remainingTime }}</span>
            ...
            <CommonButton class="invite-wins-btn" @click="showInviteWins">Convites Premiados</CommonButton>
        </div>
    </q-dialog>
</template>
<script setup>
