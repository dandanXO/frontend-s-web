...
const loadData = async () => {
  ...
  if (!getShow && store.token) {
    var res = await eventapi.get("/session/refer-wheel-spin/init?promoCode=br2-refer-wheel");
    // var res = await eventapi.get("/session/refer-wheel/init?promoCode=br2-refer-wheel");
    // res.code = 199;
    if (res.code === 0) {
      info.value = {
        ...info.value,
