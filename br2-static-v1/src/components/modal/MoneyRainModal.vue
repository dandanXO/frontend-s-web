...
const getListing = () => {
  if (store.hasToken()) {
    eventapi
      .get("/redPacketVip/list?promoCode=br2-red-packet-rain")
      .then((res) => {
        if (res.code === 0) {
...
const getNextRainTime = () => {
  eventapi
    .get("/redPacketVip/nextRainTime?promoCode=br2-red-packet-rain")
    .then((res) => {
      if (res.code === 0) {
...
.congrats-container {
  position: relative;
  background: url(../../assets/images/index/money-rain/congrats-card.png) no-repeat center center;
  background-size: contain;
  width: 100%;
  height: 470px;
  .congrats-highlight {
