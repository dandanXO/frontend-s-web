  ...
  <div class="welcome-page">
    ...
    <div class="promotionlist">
      <span v-for="(promo, i) in promoList" :key="i">
        <img @click="checkPromo(promo.promoImg)" :src="require(`../assets/welcome/promo-${promo.promoImg}.png`)" />
      </span>
    </div>
    <router-link to="/register">สมัคร</router-link>
    <div class="slogan">www.Jolly8858.com &nbsp; | &nbsp; Casino Online and Slot Game Betting</div>
  </div>
</template>
<script>
...
export default defineComponent({
  setup() {
    ...
    const checkPromo = (promo) => {
      promoList.value.forEach((el) => {
        if (promo === el.promoImg) {
          promoId.value = el.promoId;
