...
async function loadCharts() {
  ...
  const { data } = await getGameReportList(query)
  // combine from index=49 into an item where game='others'
  if (data.length > 15) {
    const others = {
      game: 'Others',
      totalBet: 0,
    ...
    data.forEach((item, index) => {
      if (index >= 15) {
        others.totalBet += item.totalBet
        others.totalWin += item.totalWin
      }
    })
    data.splice(15, data.length - 15, others)
  }
...
function getAmountChart(data, options) {
  ...
  if (data.length > 0) {
    data.forEach((item, index) => {
      ...
      dataset.push(data)
      legendData.push({ name: item.game })
    })
...
async function loadReport(first) {
  ...
  const { data: ret } = await getGameReport(query)
