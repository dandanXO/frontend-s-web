<template>
  <div class="roles-main">
    <div class="header-container">
      ...
      <div class="btn-group">
        <el-button icon="el-icon-plus" size="mini" type="primary" v-permission="['sys:member-edit:add']" @click="showDialog()">{{ t('fields.editDetails') }}</el-button>
      </div>
    </div>
    <el-dialog
    ...
    >
      ...
      >
        ...
        <div class="dialog-footer">
          <el-button @click="uiControl.memberInfoDialogVisible = false">{{ t('fields.cancel') }}</el-button>
          <el-button type="primary" @click="submitMainInfo">{{ t('fields.confirm') }}</el-button>
        </div>
      </el-form>
    </el-dialog>
    ...
    >
      ...
      >
        ...
        <el-form-item :label="t('fields.value')" prop="afterInfo">
          <el-input v-if="editLogForm.type !== 'BIRTHDAY'" style="width: 350px" v-model="editLogForm.afterInfo" />
          <el-date-picker
            v-else
        ...
        <div class="dialog-footer">
          <el-button @click="uiControl.memberEditLogDialogVisible = false">{{ t('fields.cancel') }}</el-button>
          <el-button type="primary" @click="submitEditLog">{{ t('fields.confirm') }}</el-button>
        </div>
      </el-form>
    </el-dialog>
    <el-table :data="page.records" ref="table" row-key="id" size="small" highlight-current-row>
...
import { createMemberEditLog, getMemberEditLogList, preCheckForCreate } from "../../../api/memberEditLog";
...
function submitMainInfo() {
  memberMainInfoForm.value.validate(async (valid) => {
    if (valid) {
      ...
      ElMessage({ message: t('message.found') + ret.loginName, type: "success" });
    }
  });
}
