<template>
  <div>
    <el-form ref="formRef" :model="formData" :rules="rules" class="combined-form" label-width="auto">
      ...
      <el-form-item label="代付余额通知阀值" prop="monitorSetting.setting.withdrawalLowBalanceAlertThreshold">
        <el-input-number :min="0.1" :step="0.1" v-model="formData.monitorSetting.setting.withdrawalLowBalanceAlertThreshold" />
      </el-form-item>
      ...
      <el-row justify="center" :gutter="20">
        ...
        <el-col v-if="props.mode === 'update'" :span="6">
          <TestTriggerButton title="WITHDRAW_BALANCE" />
        </el-col>
      </el-row>
    </el-form>
  </div>
...
function initializeFormData() {
  return {
    monitorSetting: {
      title: 'WITHDRAW_BALANCE',
      siteId: store.state.user.siteId,
      setting: {
        withdrawalLowBalanceAlertThreshold: 9999999.0,
...
const rules = {
  monitorSetting: {
    setting: {
      withdrawalLowBalanceAlertThreshold: [
        { required: true, message: '请填写代付余额通知阀值', trigger: 'blur' },
