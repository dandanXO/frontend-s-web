<template>
  <div>
    <el-form ref="formRef" :model="formData" :rules="rules" class="combined-form" label-width="auto">
      ...
      <el-form-item label="失败次数阈值" prop="monitorSetting.setting.failCountThreshold">
        <el-input-number :min="1" :step="1" v-model="formData.monitorSetting.setting.failCountThreshold" />
      </el-form-item>
      <el-form-item label="失败率阈值" prop="monitorSetting.setting.failRatePercentageThreshold">
        <el-input-number :min="1" :step="1" v-model="formData.monitorSetting.setting.failRatePercentageThreshold" />
      </el-form-item>
      <el-form-item label="检查分钟数" prop="monitorSetting.setting.checkDurationMinutes">
        <el-input-number :min="1" :step="1" v-model="formData.monitorSetting.setting.checkDurationMinutes" />
      </el-form-item>
  ...
  return {
    monitorSetting: {
      ...
      setting: {
        ...
        failRatePercentageThreshold: 10,
        checkDurationMinutes: 10,
      },
      status: 1,
    },
    notificationSetting: {
...
const rules = {
  monitorSetting: {
    setting: {
      failCountThreshold: [
        { required: true, message: '失败次数阈值必填', trigger: 'blur' },
        { type: 'number', min: 1, message: '最小值为1', trigger: 'blur' }
      ],
      failRatePercentageThreshold: [
        { required: true, message: '失败率阈值必填', trigger: 'blur' },
        { type: 'number', min: 1, message: '最小值为1', trigger: 'blur' }
      ],
      checkDurationMinutes: [
        { required: true, message: '检查分钟数必填', trigger: 'blur' },
        { type: 'number', min: 1, message: '最小值为1', trigger: 'blur' }
      ],
    },
    status: [
