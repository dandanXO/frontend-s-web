<template>
  <div class="roles-main">
    <div class="header-container">
      <div class="search">
        ...
        <el-button
          ...
          size="mini"
          type="primary"
          @click="loadReferFriendRebateRecords()"
        ...
        <div class="btn-group">
          ...
          <el-button
            ...
            size="mini"
            type="warning"
            v-permission="['sys:refer-rebate-record:export']"
            @click="exportExcel"
    ...
    <el-card class="box-card" shadow="never" style="margin-top: 20px">
      <template #header>
        <div class="clearfix">
          <span class="role-span">{{ t('fields.referFriendRebateRecords') }}</span>
        </div>
      </template>
      ...
      <el-pagination
        ...
        @size-change="loadReferFriendRebateRecords()"
      />
    </el-card>
  </div>
...
import { getReferFriendRebateRecordDetails } from '@/api/refer-friend-rebate-record-detail';
...
const EXPORT_HEADER = [t('fields.loginName'), t('fields.site'), t('fields.betAmount'), t('fields.rebateAmount'), t('fields.status'),
  t('fields.rebateDistributeTime'), t('fields.distributeBy'), t('fields.distributeTime'), t('fields.updateBy'), t('fields.updateTime')];
...
async function loadReferFriendRebateRecords() {
  page.loading = true;
...
async function showDetails(record) {
  uiControl.dialogTitle = t('fields.referFriendRebateDetails')
  ...
  const { data: ret } = await getReferFriendRebateRecordDetails(query);
...
async function adjust() {
  adjustForm.value.validate(async valid => {
    if (valid) {
      ...
      await loadReferFriendRebateRecords()
    }
  });
}
...
async function exportExcel() {
  ...
  wb.SheetNames.push('Refer_Friend_Rebate_Record');
  wb.Sheets.Refer_Friend_Rebate_Record = ws;
  XLSX.writeFile(wb, "refer_friend_rebate_record.xlsx");
