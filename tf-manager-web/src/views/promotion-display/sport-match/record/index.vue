<template>
  <div class="roles-main">
    <div class="header-container">
      ...
      <div class="btn-group">
        <el-button
          size="mini"
          type="success"
          v-permission="['sys:privi:sport-match-record:update']"
    ...
    >
      ...
      <el-table-column :label="t('fields.operate')" align="center" v-if="!hasRole(['SUB_TENANT']) && hasPermission(['sys:privi:sport-match-record:update'])" fixed="right" width="280">
        <template #default="scope">
          <el-button
...
import { getSportMatchRecord, settleSportMatchRecord, cancelSportMatchRecord, settleAllSportMatchRecord } from "@/api/sport-match";
...
async function loadMatchRecord() {
  ...
  const { data: ret } = await getSportMatchRecord(query);
...
async function settle(id) {
  ElMessageBox.confirm(t('message.confirmSettlement'), {
    ...
    await settleSportMatchRecord(id)
...
async function cancel(id) {
  ElMessageBox.confirm(t('message.confirmCancel'), {
    ...
    await cancelSportMatchRecord(id)
    await loadMatchRecord()
...
async function settleAllPending() {
  ElMessageBox.confirm(t('message.confirmSettlement'), {
    ...
    await settleAllSportMatchRecord(request.siteId)
    await loadMatchRecord()
    ElMessage({ message: t('message.settled'), type: 'success' })
  })
}
