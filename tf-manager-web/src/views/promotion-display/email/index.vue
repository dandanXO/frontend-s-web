  ...
  <div class="roles-main">
    ...
    >
      ...
      <el-table-column
        ...
        v-if="
          !hasRole(['SUB_TENANT']) &&
          (hasPermission(['sys:email:update']) ||
            hasPermission(['sys:email:del']))
        "
      >
        <template #default="scope">
          <el-button
...
const EXPORT_EMAIL_LIST_HEADER = [
  'Email Address',
  'Site ID (Refer Mapping Sheet)',
  'Bonus Amount',
]

const IMPORT_EMAIL_LIST_JSON = ['email', 'siteId', 'bonusAmount']
...
const formRules = reactive({
  email: [required(t('message.validateEmailRequired'))],
...
function handleSelectionChange(val) {
  chooseEmail = val
  if (chooseEmail.length === 0) {
    uiControl.editBtn = true
    uiControl.removeBtn = true
  } else if (chooseEmail.length === 1) {
...
async function loadEmails() {
  page.loading = true
  const { data: ret } = await getEmails(request)
...
function showDialog(type) {
  if (type === 'CREATE') {
    ...
    uiControl.dialogTitle = t('fields.addEmail')
    form.id = null
    form.privilegeStatus = 1
    form.siteName = null
  } else if (type === 'EDIT') {
    uiControl.dialogTitle = t('fields.editEmail')
  }
...
async function removeEmail(email) {
  ElMessageBox.confirm(
    ...
    if (email) {
      ...
      await deleteEmail(chooseEmail.map(u => u.id))
    }
    await loadEmails()
...
async function downloadTemplate() {
  const exportEmailList = [EXPORT_EMAIL_LIST_HEADER]
  const maxLengthEmailList = []
  const wsEmailList = XLSX.utils.aoa_to_sheet(exportEmailList)
  setWidth(exportEmailList, maxLengthEmailList)
  const wsEmailListCols = maxLengthEmailList.map(w => {
    return { width: w }
  })
  wsEmailList['!cols'] = wsEmailListCols
  ...
  wb.SheetNames.push('Email_List')
  wb.Sheets.Email_List = wsEmailList
  wb.SheetNames.push('Mapping')
  wb.Sheets.Mapping = wsMapping
  XLSX.writeFile(wb, 'email_list.xlsx')
}
...
async function confirmImport() {
  ...
  do {
    if (records.length > 10000) {
      await createBatchEmail(records.slice(0, 10000));
      records.splice(0, 10000);
    } else {
      await createBatchEmail(records);
