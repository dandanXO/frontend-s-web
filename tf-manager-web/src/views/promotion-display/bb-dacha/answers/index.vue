<template>
  <div class="roles-main">
    ...
    >
      <el-table-column prop="quizTitle" :label="t('fields.title')" width="280" />
...
const bbDachaAnswerForm = ref(null);
...
function showDialog(type) {
  if (bbDachaAnswerForm.value) {
    bbDachaAnswerForm.value.resetFields();
  }
  if (type === "UPDATE") {
...
function updateStatus() {
  bbDachaAnswerForm.value.validate(async (valid) => {
    if (valid) {
      await updateBbDachaAnswer(form.id, form.status);
      uiControl.dialogVisible = false;
      await loadBbDachaAnswer();
