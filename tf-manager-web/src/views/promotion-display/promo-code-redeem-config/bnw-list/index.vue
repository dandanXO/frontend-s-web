  ...
  <div class="roles-main">
    <div class="header-container">
      <div class="search">
        ...
        <el-select
          clearable
          v-model="request.sitePrivilegeId"
          size="small"
    ...
    <el-card class="box-card" shadow="never" style="margin-top: 40px">
      <template #header>
        <div class="clearfix">
          <span class="role-span">{{ t('fields.memberPrivilegeBlacklist') }}</span>
        </div>
      </template>
    ...
    >
      ...
      >
        ...
        <el-form-item :label="t('fields.privilege')" prop="sitePrivilegeId">
          <el-select
            v-model="form.sitePrivilegeId"
            :placeholder="t('fields.privilege')"
            style="width: 350px;"
            filterable
            default-first-option
      ...
      <div v-else>
        ...
        >
          ...
          <el-table-column prop="sitePrivilegeId" :label="t('fields.privilegeId')" width="330" />
          <el-table-column prop="isBlacklist" :label="t('fields.status')" min-width="150">
          <template #default="scope">
            <el-tag v-if="scope.row.isBlacklist === 0" type="info">{{ t('priviCodeRedeemBnWStatus.whitelist') }}</el-tag>
            <el-tag v-if="scope.row.isBlacklist === 1" type="danger">{{ t('priviCodeRedeemBnWStatus.blacklist') }}</el-tag>
...
import { createBatchBlacknWhitelist, createBlacknWhitelist, deleteBlacknWhitelist, getBlacknWhitelist, getPromoCodeExcelMapping } from "../../../../api/privi-code-redeem-bnw-list";
...
const EXPORT_LIST_HEADER = [
  'Login Name',
  'Privilege ID (Refer Mapping Sheet)',
...
async function loadPrivilegeInfos(siteId) {
  if (siteId) {
    const { data: privilegeInfo } = await getPromoCodeExcelMapping(siteId);
    requestPriviList.list = privilegeInfo;
  } else {
    const { data: privilegeInfo } = await getPromoCodeExcelMapping();
    requestPriviList.list = privilegeInfo;
  }
}
...
async function loadFormSelect() {
  form.memberId = null;
  form.sitePrivilegeId = null;
  await loadPrivilegeNamesBySiteId();
}

async function loadPrivilegeNamesBySiteId() {
  if (form.siteId) {
    const { data: privilegeInfo } = await getPromoCodeExcelMapping(request.siteId);
...
async function downloadTemplate() {
  ...
  wb.SheetNames.push('RedeemCode_black_and_whitelist');
  wb.Sheets.RedeemCode_black_and_whitelist = wsBlacklist;
  wb.SheetNames.push('Mapping');
  wb.Sheets.Mapping = wsMapping;
  XLSX.writeFile(wb, 'RedeemCode_black_and_whitelist.xlsx');
}
...
async function confirmImport() {
  ...
  do {
    if (records.length > 10000) {
      await createBatchBlacknWhitelist(records.slice(0, 10000));
      records.splice(0, 10000);
    } else {
      await createBatchBlacknWhitelist(records);
