...
async function toBeforePaid() {
  await fromPayToBeforePaid(chooseRecord.map(a => a.id))
  await loadRecord()
...
async function success() {
  ...
  if (uiControl.dialogType === 'PAY') {
    await fromPayToSuccess(payForm.id, payForm.bankCard)
    ElMessage({ message: t('message.paySuccess'), type: 'success' })
  } else if (uiControl.dialogType === 'AUTOPAY') {
    await fromPayToAutopay(payForm.id, payForm.withdrawPlatformId)
...
async function fail() {
  toFailForm.value.validate(async valid => {
    if (valid) {
      clicked.fail = true
      await fromPayToFail(failForm.id, failForm.reasonType, failForm.failReason)
      uiControl.dialogVisible = false
      clicked.fail = false
      await loadRecord()
