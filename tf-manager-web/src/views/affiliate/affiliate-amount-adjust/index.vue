<template>
  <div class="roles-main">
    <div class="header-container">
      <div class="search">
        ...
        <el-button
          ...
          size="mini"
          type="success"
          @click="loadAffiliateAmountAdjust"
        >
          {{ t('fields.search') }}
        </el-button>
        <el-button
      ...
      <div class="btn-group">
        <el-button
          ...
          size="mini"
          type="primary"
          v-permission="['sys:affiliate:amount:adjust:add']"
        ...
        <el-button
          ...
          size="mini"
          type="danger"
          v-permission="['sys:affiliate:amount:adjust:deduct']"
        ...
        <el-button
          ...
          size="mini"
          type="success"
          v-permission="['sys:affiliate:amount:adjust:import']"
        ...
        <el-button
          size="mini"
          type="primary"
          v-permission="['sys:affiliate:amount:adjust:export']"
    ...
    >
      ...
      <el-table-column prop="type" :label="t('fields.type')" min-width="80">
...
import {
  createAddAffiliateAmountAdjust,
  createDeductAffiliateAmountAdjust,
  getAffiliateAmountAdjust,
  ...
  getExport,
} from '../../../api/affiliate-amount-adjust'
...
const site = ref(null)
const affiliateAmountAdjustForm = ref(null)
...
const loginNameValidator = async (rule, value, callback) => {
  if (uiControl.dialogType === 'CREATE_DEDUCT') {
    const { data: bal } = await getAffiliateBalanceByWalletOrCommission(
      value,
      form.siteId,
      form.mainWallet
    )
    if (bal === null || bal === undefined) {
      callback(new Error(t('message.affiliateNotInSite')))
    } else {
  ...
  return (
    time.getTime() <
      moment(new Date())
        .subtract(2, 'months')
        .startOf('month')
        .format('x') || time.getTime() > moment(new Date()).add(1, 'day')
  )
}
...
async function loadAffiliateAmountAdjust() {
  page.loading = true
...
function createAdd() {
  affiliateAmountAdjustForm.value.validate(async valid => {
    form.id = null
    form.memberId = null
    const { data: id } = await findIdByLoginName(form.loginName, form.siteId)
    form.memberId = id
    if (valid) {
      await createAddAffiliateAmountAdjust(form)
      uiControl.dialogVisible = false
      await loadAffiliateAmountAdjust()
...
function createDeduct() {
  affiliateAmountAdjustForm.value.validate(async valid => {
    ...
    form.memberId = id
    if (valid) {
      await createDeductAffiliateAmountAdjust(form)
      uiControl.dialogVisible = false
      await loadAffiliateAmountAdjust()
      ElMessage({ message: t('message.addSuccess'), type: 'success' })
    }
...
async function downloadTemplate() {
  ...
  wb.SheetNames.push('Affiliate_Amount_Adjust')
  wb.Sheets.Affiliate_Amount_Adjust = wsAmountAdjust
    ...
    if (valid) {
      ...
      Object.entries(recordCopy).forEach(([key, value]) => {
        ...
        if (value) {
          ...
          item.mainWallet = importForm.mainWallet
