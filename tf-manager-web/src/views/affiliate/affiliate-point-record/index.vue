...
import { getAffiliatePointRecordList, addAffiliatePointRecord } from '../../../api/member-point-record'
...
async function loadMemberPointRecord() {
  ...
  const { data: ret } = await getAffiliatePointRecordList(query)
...
function submit() {
  memberPointForm.value.validate(async valid => {
    if (valid) {
      await addAffiliatePointRecord(form)
