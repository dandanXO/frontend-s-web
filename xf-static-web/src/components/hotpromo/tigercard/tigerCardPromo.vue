  ...
  <el-dialog align-center v-model="isGiftModal" title="赠送虎卡" width="500">
    <el-form :rules="sendRules" ref="hukaFormRef" style="padding: 20px;" :model="form">
      ...
      <div style="text-align: center">
        <el-button class="common-btn grey" color="#ff0000" @click="resetRegForm(hukaFormRef)">
          重置
        </el-button>
...
const pageInit = () => {
  tigerCardInit({promoCode: "xf1-tiger-card"}).then((res) => {
    if(res.code === 0) {
...
const getNewTigerCard = () => {
  ...
  getMemberCard({promoCode: "xf1-tiger-card"}).then((res) => {
    if(res.code === 0) {
...
const compoundCard = () => {
  ...
  synthesisCard({promoCode: "xf1-tiger-card"}).then((res) => {
    if(res.code === 0) {
      pageInit()
      ElMessage.success({
        type: "success",
