export default {
  common: {
    title: "代理系统",
    logout: "登出",
    changePassword: "更改密码"
  },
  fields: {
    account: "账号",
    accountInfo: "账号信息",
    activeMember: "活跃会员",
    activeUsers: "活跃会员",
    add: "新增",
    addBankCard: "新增银行卡",
    addVirtualCard: "新增虚拟卡",
    adjustment: "调整金额",
    affiliate: "代理",
    affiliateCode: "代理代码",
    affiliateLevel: "代理级别",
    affiliateStatus: "代理状态",
    amountOfFirstDeposit: "首充金额",
    balance: "余额",
    bank: "银行卡",
    bankId: "银行ID",
    bet: "下注金额",
    bonus: "奖金",
    cancel: "取消",
    cardAccount: "银行卡账号",
    cardAddress: "银行卡地址",
    cardNumber: "银行卡号码",
    commission: "佣金",
    commissionRate: "佣金比率",
    confirm: "确认",
    confirmNewPassword: "确认新密码",
    confirmWithdraw: "确认提款",
    copy: "复制",
    createAffiliate: "新增代理",
    crypto: "数字货币",
    currentPassword: "当前密码",
    deposit: "存款",
    depositAmount: "存款金额",
    depositCount: "存款次数",
    downlineAffiliate: "直属代理",
    downlineMember: "直属会员",
    download: "下载",
    editAffiliate: "编辑代理",
    editRealName: "编辑真实名字",
    email: "邮箱",
    endDate: "结束日期",
    enterTheWithdrawalAmount: "请输入提款金额",
    firstDepositUsers: "首充人数",
    lastMonth: "上月",
    lastWeek: "上周",
    link: "链接",
    loginName: "用户名",
    member: "会员",
    myAccount: "我的账号",
    realName: "真实名字",
    name: "名字",
    newPassword: "新密码",
    newUsers: "新注册会员人数",
    ngr: "NGR",
    noData: "查无数据",
    operate: "操作",
    password: "密码",
    payout: "支出",
    profit: "利润",
    recordTime: "记录时间",
    reenterPassword: "重新输入密码",
    referralCode: "推荐码",
    referralLink: "推荐链接",
    registerTime: "注册时间",
    reset: "重置",
    revenueShare: "收入",
    revenueShareRate: "收入比率",
    search: "搜索",
    select: "选择",
    selectACard: "选择银行卡",
    selectBankCard: "选择银行卡",
    site: "站点",
    startDate: "开始日期",
    status: "状态",
    telephone: "电话号码",
    thisMonth: "本月",
    thisWeek: "本周",
    today: "今日",
    totalDeposit: "总存款",
    totalDownlineAffiliate: "总直属代理",
    totalDownlineMember: "总直属会员",
    totalWithdraw: "总提款",
    transferIn: "转入",
    transferOut: "转出",
    visitsNo: "访问量",
    winLoss: "输赢",
    withdraw: "提款",
    withdrawAmount: "提款金额",
    withdrawCount: "提款次数",
    withdrawalAmount: "提款金额",
    yesterday: "昨日"
  },
  message: {
    addSuccess: "新增成功",
    chineseCharacters: "请输入中文字符",
    editSuccess: "编辑成功",
    inputRealName: "会员真实名字未填",
    emailFormat: "请输入正确的邮箱格式",
    inputDigits: "请输入数字",
    inputPassword: "请输入密码",
    inputPasswordAgain: "请再输入密码",
    length6To12: "字符串必须为 6 至 12 字",
    length16To19: "长度应为 16-19",
    passwordLength: "密码必须多于 6 字并少于 12 字",
    redirectBankDeposit: "您已被重定向至您的特定银行以继续存款。存款成功后会在此处体现。",
    reenterPassword: "请重新输入密码",
    referralLinkCopied: "推荐链接已复制到剪贴板",
    remaining: "剩余次数",
    requiredAffiliateLevel: "代理级别必填",
    requiredAmount: "金额必填",
    requiredCardAccount: "银行卡账号必填",
    requiredCardAddress: "银行卡地址必填",
    requiredCardNumber: "银行卡号码必填",
    requiredCommission: "佣金必填",
    requiredEmail: "邮箱必填",
    requiredLoginName: "用户名必填",
    requiredPassword: "密码必填",
    requiredRealName: "请输入姓名",
    requiredRevenueShare: "收入必填",
    requiredTelephone: "电话号码必填",
    requiredUserName: "用户名必填",
    selectBankCard: "请选择银行卡",
    singleLimit: "单笔限额",
    success: "成功",
    times: "次",
    twoPasswordNotMatch: "重新输入密码不相同",
    unbindConfirm: "请确认是否要解绑 ",
    validateCommission: "佣金必须介于 0 至 1 之间",
    validateRevenueShare: "收入必须介于 0 至 1 之间",
    withdrawalToday: "今日提款"
  },
  menu: {
    undefined: "",
    Dashboard: "仪表盘",
    Team: "团队",
    Member: "会员",
    Affiliate: "代理",
    Statistics: "统计",
    "Game Stats": "游戏统计",
    "Affiliate Daily Report": "代理每日报表",
    "Affiliate Center": "代理中心",
    "Bind Bank Cards": "绑定银行卡",
    "Bank Withdrawal": "银行提款",
  },
  error: {
    400: "请求无效",
    403: "禁止访问",
    405: "方法不被允许",
    500: "系统错误",
    501: "请求ID重复",
    502: "无法找到伺服器",
    504: "请求太频繁",
    601: "令牌验证失败",
    602: "令牌已逾期",
    603: "无法找到令牌",
    604: "账号已登录",
    707: "余额不足",
    800: "验证码失败",
    801: "验证码已失效",
    901: "代理记录不存在",
    902: "此用户非代理",
    905: "真实名字尚未填写",
    1000: "登录名或密码错误",
    1001: "登录失败, 账号已被禁用",
    1308: "余额不足",
    13000: "域名不存在",
    14000: "用户名已被使用 : ",
    14001: "电话号码已被使用 : ",
    14002: "邮箱已被使用 : ",
    14003: "代理代码不存在 : ",
    14004: "代理代码不属于此站点 : ",
    14005: "代理佣金必须少于上线代理佣金 : ",
    14006: "代理收入必须少于上线代理收入 : ",
    14007: "代理申请尚未被批准",
    14008: "代理佣金必须多余子代理佣金 : ",
    14009: "代理收入必须多余子代理收入 : ",
    15201: "旧密码不正确",
    15202: "新密码不能与旧密码相同"
  }
};
