...
const onResponse = response => {
  ...
  if (res.code !== ResponseCode.SUCCESS) {
    ...
    if (
      res.code === ResponseCode.ERROR_FORBIDDEN ||
      res.code === ResponseCode.ERROR_FORBIDDEN2
    ) {
      if (res.data) {
        ...
        ElMessage({
          message:
            i18n.global.t('error.' + res.code) +
              (res.data && res.data.parameter ? res.data.parameter : '') ||
            'Error',
          type: 'error',
        })
      }
...
const https = api => {
  ...
  if (
    ...
    currentHost.indexOf('br2-') > -1 ||
    currentHost.indexOf(br2Host) > -1 ||
    currentHost.indexOf('br1-') > -1 ||
    currentHost.indexOf(br1Host) > -1
  ) {
    apiUrl = isAff
      ? process.env.VUE_APP_BR2_RST_API
      : isCr
      ? process.env.VUE_APP_BR2_CR_API
      : process.env.VUE_APP_BR2_AFF_API
  } else {
    apiUrl = isAff
