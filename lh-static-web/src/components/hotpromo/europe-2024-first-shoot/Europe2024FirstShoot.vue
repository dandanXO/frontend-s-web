  ...
  <div class="europe-container">
    <div class="tab-content">
      <div class="sched-buttons">
        <button
          v-for="(tab, index) in schedTabs"
          :key="index"
          @click="activateSchedTab(index)"
          :class="{ active: schedActiveTab === index }"
        >
          {{ tab.name }}
        </button>
      </div>
...
import NewComersLimeLight from "@/components/hotpromo/europe-2024-first-shoot/components/newcomerslimelight.vue";
import EmergencyFund from "@/components/hotpromo/europe-2024-first-shoot/components/emergencyfund.vue";
