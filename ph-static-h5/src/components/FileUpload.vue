<template>
  <q-file name="upload_img" v-model="file" class="q-pt-md" filled label="Upload image" color="white">
...
export default defineComponent({
  ...
  setup: (props, { emit }) => {
    ...
    var rstApi = process.env.RST_API;
        ...
        try {
          ...
          if (data.code === 0) {
            ...
            $q.notify({
              type: "negative",
              position: "top",
              message: `${file.value.name} file upload failed. Upload limit, please upload images later.`,
              icon: "report_problem"
            });
            file.value = null;
          }
