<template>
    <q-table dense class="datatable" :grid="$q.screen.lt.md" :loading="props.loading" :columns="props.tableColumns"
        :rows="props.dataState" :no-data-label="$t('lang.no_data_label')"
        :rows-per-page-label="$t('lang.row_per_page_label')" :row-key="props.rowKey || 'serialNumber'"
        :rows-per-page-options="[props.rowsPerPage || 10]" :hide-pagination="true">
    ...
    <div class="datatable-pagination" v-if="props.pagination && props.pagination.pages > 0">
        ...
        <q-pagination :modelValue="props.pagination.current" :max="props.pagination.pages" :max-pages="10"
