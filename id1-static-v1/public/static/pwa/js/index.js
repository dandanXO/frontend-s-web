    ...
    case "INSTALL":
      ...
      if (!deferredPrompt) {
        ...
        if (isIos) {
          ...
          window.open("https://idn.55ace.com/register", "_blank");
        }
      } else {
...
function openLinkInPreferredBrowser(url) {
  ...
  if (isIos) {
    ...
    const fallbackTimer = setTimeout(() => {
      ...
      window.open(
        `https://vpfzg.cc/?p0=${affiliateCodePwa}&p1={{campaign.name}}&p2={{campaign.id}}&p3={{adset.name}}&p4={{adset.id}}&p5={{ad.name}}&p6={{ad.id}}`,
        "_self"
      );
    }, 1500);
