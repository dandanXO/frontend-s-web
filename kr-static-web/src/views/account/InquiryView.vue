<template>
        ...
        <div class="feedback-container">
          <div class="feedback-list-wrapper">
            ...
            <div class="feedback-list">
              ...
              <template v-else>
                <div v-for="item in inquiriesList.records" :key="item.page"
                  @click="readFeedback(item.id)" class="feedback" :class="item === selected ? 'active-feedback' : ''">
                  ...
                  <div class="info-wrapper">
                    <div><span class="date-time">{{ getLocaleDateTime(item.createTime)
                        }}</span></div>
                    <div><span class="caption" v-if="item.hasOwnProperty('readTime')"
                        style="font-size:10px;" :style="!item.readTime ? 'color:#808080' : 'color:#FFC000'">{{
                          !item.replyId ? $t('feedback.feedback_waiting_admin_reply') : $t('feedback.feedback_admin_replied')
                        }}</span></div>
                  </div>
                </div>
              </template>
              ...
              <div>
                  <div class="feedback-actions">
                    <el-button-group flat>
                      <el-button type="primary" v-if="selectedMessages?.length" 
                        ...
                        {{ $t('feedback.feedback_delete_selected') + (selectedMessages?.length ? `(${selectedMessages.length})` : '') }}
                        </el-button>
                    </el-button-group>
                  </div>
                </div>
                <template v-if="selected">
                  <div class="feedback-content" v-for="inquiry, index in [selected, ...repliesOfInquiries]" :key="index">
                    ...
                    <span class="date-time" v-if="inquiry.readTime">{{ $t('feedback.feedback_read_at') }} {{
  ...
  const initOutbox = () => {
    ...
    Promise.all([server.REST.get("/session/feedback/sysReply"), server.REST.get('/session/feedback/replies')]).then(([inquiriesRes, replyInquiriesRes]) => {
  ...
  const readFeedback = (id, showReadNotify = true) => {
    ...
    if (!currentMail?.content) {
      ...
      server.REST.get(`/session/feedback/${id}/read`).then((res) => {
        const { code, data } = res
